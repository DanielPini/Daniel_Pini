...
export default class Tapper {
    ...
    createElements() {
        ...
        tapperWrapper.append(tempoSection, tempoChangeContainer, tempoInput, commonTemposContainer, tempoTapper, resultsDiv);
   ...
   ${delta > 0
        ...
        (_a = this.resultsDiv) === null || _a === void 0 ? void 0 : _a.append(resultsTitle, resultsText);
    }
    displayFailure() {
        var _a;
        const resultsTitle = document.createElement("h1");
   ...
   Not enough taps to calculate. <br />Please try tapping more quickly.`;
    ...
    registerTaps(tapButton) {
        if (!tapButton) {
            console.error("Tap button not found. Ensure the element is passed correctly.");
            return;
        }
        const handleTap = () => {
            if (this.tapCount === 0) {
            ...
            this.timeoutId = setTimeout(() => {
                if (this.tapCount > 1) {
                    ...
                    const actualAverageTime = (this.lastTapTime - this.firstTapTime) / (this.tapCount - 1);
